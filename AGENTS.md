...
- **Submodule image workflow** — body images go in `src/assets/img/<article-slug>/`; `covers/` is only for article covers or unrelated fallback artwork. Commit and push the image submodule first, then commit the parent pointer. → [details](./gotcha.md#submodule-image-commit-order)
