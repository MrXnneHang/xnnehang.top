...
- **炼金台 Todo Issue follow-ups** — create a public-safe Todo Issue for discrete, valuable work discovered outside the user's requested scope; do not implement it as part of the current task. → [details](./gotcha.md#炼金台-todo-issue-follow-ups)
- **Submodule image workflow** — body images go in `src/assets/img/<article-slug>/`; `covers/` is only for article covers or unrelated fallback artwork. Image-only submodule changes are committed and pushed directly to `image-hosting` `main`; parent content/code and submodule-pointer changes follow the normal PR workflow. → [details](./gotcha.md#submodule-image-commit-order)
- **Preserve first-image covers** — if the article already starts with its intended cover, keep it as the first body image; do not move it to frontmatter or manually convert its format. → [details](./gotcha.md#preserve-the-first-body-image-as-the-derived-cover)
...
- **YAML frontmatter prose fields** — always double-quote article `title` and `description`; never rely on YAML plain-scalar parsing, especially when values may contain `:` or other special characters. → [details](./gotcha.md#yaml-frontmatter-prose-fields)
- **Fixed post taxonomy** — `category` is exactly one of `technology`, `culture`, `thought`, `life`; `kind` is exactly one of `tutorial`, `review`, `reflection`, `learning-note`, `resource`, `note`. Never invent values while publishing or translating. Choose Category by where the article ultimately belongs and Kind by its writing form/reader promise; bilingual pairs keep the same internal keys. → [details](./gotcha.md#fixed-post-taxonomy)
- **Bilingual shelf fields** — preserve Chinese `shelf` and `subCategory` keys in every locale because the shelf UI localizes them separately. → [details](./gotcha.md#fixed-post-taxonomy)
