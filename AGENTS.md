...
- **Bilingual taxonomy fields** — localize `category` to the article language using the established five-category taxonomy; preserve Chinese `shelf` and `subCategory` keys in every locale because the shelf UI localizes them separately. → [details](./gotcha.md#bilingual-taxonomy-fields)
