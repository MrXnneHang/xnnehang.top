...
:::note[Opus]
**只说对了一半：上层包含下层代码是真的，“下层变化会实时自动同步到上层”则不是真的。**
...
现在假设最底层的 PR A 又新增了一个提交 `A1`。A 的 branch 向前移动了，但 B 到 E 不会实时跟着移动，而是暂时分叉成这样：
...
这里的 `B1` 到 `E1` 是 rebase 后的新提交，虽然某一层的代码可能没有发生实质变化，但 commit SHA 会被重写。之后还需要把更新后的 branches push 到远端。也可以直接执行：
...
真正可能变麻烦的是**冲突，而不是 sync 命令本身**。级联 rebase 时，每一层都要在它的新下层之上重放提交：A 的改动如果同时影响了 B、C、D、E 修改的代码，那么这些层都有可能依次产生冲突。`gh stack sync` 遇到 rebase 冲突时不会留下同步了一半的 stack，而是恢复各 branch 原来的状态，并提示改用 `gh stack rebase` 处理；解决当前冲突、暂存文件后执行 `gh stack rebase --continue`，如果后面的层再次冲突，就要继续处理。
...
所以 Stacked PRs **确实解决了手动逐层对齐的问题，但不是让同步消失了，而是把它收敛成一个知道整条依赖链的级联操作**：没有冲突时，多级 sync 并不麻烦；发生冲突时，仍要按照层级逐个解决。实践中应该尽量让每层改动聚焦、减少跨层修改同一片代码，而不是无限增加 Stack 的深度。
:::

非常清晰了，也就是说 sync 依然是开发者或者 Agent 手动执行的，但是比起以前复杂的 rebase diff fetch 的操作，目前非常地简单，只需要运行一条命令即可。但是对于冲突的部分，依然需要手动解决。
